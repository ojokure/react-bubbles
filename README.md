...
encoded token signatures, 

Authenticate users by token/cookies,

 Protect private components with Privateroutes (that can only be accessed upon authentication)

- [ ] Describe how web servers work.

 Web server is a program that uses HTTP (Hypertext Transfer Protocol) to serve 
the files that form Web pages to users, in response to their requests, which are forwarded by their computers' HTTP clients. 
